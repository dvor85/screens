...
class Kbdsvc():
    ...
    def __call__(self, *args, **kwargs):
        ...
        if len(compname) > 0 and len(username) > 0 and len(filename) > 0:
            ...
            self.kbddir = os.path.join(self.datadir, 'kbdsvc')
#             self.datadir = os.path.join(config['DATA_DIR'], fmt('{dt}/{comp}/{user}',
#                                                                    dt=filename[0:8],
#                                                                    comp=compname,
#                                                                    user=username))
            return self.store(os.path.join(self.kbddir, filename), data)
